...
$('#Form_comunidad_public').submit(function () {
	var $user_id_visit = document.querySelector('#published_user_edit_id').value 
	var $user_id_onwer = document.querySelector('#published_user_id').value

	if($user_id_visit === $user_id_onwer) {
		console.log('Evento publick ocurriendo')
...
socket.on('comunidad_publish', function (content) {
	...
	if(content.user_id === $user_id) {
		...
		if(content.publish_multimedia !== null) {
			...
			// Validar el template de ruta de imagen a la altura o darle una ubicacion de root de path
...
// Evento click btn_answer_like ---------------- EN PROCESO
