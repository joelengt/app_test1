myApp.controller('cursosCompletadosController', ['$scope', '$http', '$location', function($scope, $http, $location){
	$http({
		method: 'GET',
		url: '/plataforma'
	}).then(function(res){
		...
		if (res.data.user.access === 'premium') {
			...
			for (var i = 0; i < $scope.myCourses.length; i++) {
				if ($scope.myCourses[i].progreso.answer_correct >= $scope.myCourses[i].progreso.questions_max) {
					if ($scope.myCourses[i].progreso.questions_max > 0) {
						$scope.coursesComplete.push($scope.myCourses[i])
					}
				}
			}
			// console.log($scope.coursesComplete)
		} else {
